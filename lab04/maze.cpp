...
void Maze::solve()
{
    solve(1, 1);     // Uncomment this line if using the 
                        // solve(int, int) implementation

    // solve(Position(1, 1));   // Uncomment this line if using the
                                // solve(Position) implementation
...
bool Maze::solve(int row, int col)
{
    ...
    // cout << pos << " at row " << row << " col " << col;
    if (pos == FINISH) { 
        // cout << "found the Finish" << endl;
        return true; 
    }
    else if (pos == WALL || pos == PATH || pos == REJECT) {
        return false;
    }
    // cout << "...placing dot at: " << row << col << endl;
    pos = PATH;
    ...
    // cout << "position " << pos << " at " << row << col << endl;
    return true;
}
...
{
    if (row == maze_array_size - 1) return 0;
    int dots = num_dots_in_row(row, 1);
    cout << "there are " << dots << " dots in row " << row << endl;
    return dots + num_dots_by_rows(row + 1);
...
{
    if (col == maze_array_size - 1) return 0;
    int dots = 0;
    if (maze[row][col] == PATH) { dots++; }

    return dots + num_dots_in_row(row, col + 1);
}
